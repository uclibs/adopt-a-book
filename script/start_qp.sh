...
export RAILS_RELATIVE_URL_ROOT=/adopt_a_book
export RAILS_ENV=production

bin/bundle exec rake db:seed

# Check if puma is started
if ! ( [ -f /opt/webapps/adopt_a_book/current/tmp/puma/pid ] && pgrep -F /opt/webapps/adopt_a_book/current/tmp/puma/pid > /dev/null )
then
    bin/bundle exec puma -d
else
    bin/bundle exec pumactl stop
    sleep 10
    cd /opt/webapps/adopt_a_book/current
    bin/bundle exec puma -d
fi
